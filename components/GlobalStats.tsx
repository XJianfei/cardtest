...
import { ArrowLeft, CheckCircle2, Circle, PieChart, AlertCircle, LayoutGrid } from 'lucide-react';
...
export const GlobalStats: React.FC<GlobalStatsProps> = ({ decks, onBack }) => {
  ...
  decks.forEach(deck => {
    ...
    masteredCount += deck.cards.filter(c => c.mastered).length;
  });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Global Progress</h2>
          <p className="text-slate-500 text-sm">Summary across all decks</p>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center min-h-[300px]">
           <div className="relative w-48 h-48">
             ...
             <div className="absolute inset-0 flex flex-col items-center justify-center">
               ...
               <span className="text-xs text-slate-400 uppercase tracking-wide font-semibold">Total Mastered</span>
             </div>
           </div>
      ...
      {learningCount === 0 ? (
        <div className="p-8 bg-green-50 rounded-2xl border border-green-100 text-center">
          ...
          <h4 className="text-lg font-semibold text-green-800">Everything is mastered!</h4>
          <p className="text-green-600">Great job, you have no pending cards across any deck.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {decks.map(deck => {
            const learningCards = deck.cards.filter(c => !c.mastered);
            if (learningCards.length === 0) return null;

            return (
              <div key={deck.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="bg-slate-50 px-4 py-3 border-b border-slate-100 flex items-center gap-2">
                   <div className={`w-6 h-6 rounded bg-${deck.themeColor || 'indigo'}-100 text-${deck.themeColor || 'indigo'}-600 flex items-center justify-center`}>
                     <LayoutGrid className="w-3.5 h-3.5" />
                   </div>
                   <h4 className="font-semibold text-slate-700">{deck.title}</h4>
                   <span className="text-xs font-medium bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full ml-auto">
